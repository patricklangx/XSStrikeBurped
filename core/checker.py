...
def checker(url, params, headers, GET, request_type, delay, payload, positions, timeout, encoding):
    checkString = 'st4r7s' + payload + '3nd'
    if encoding:
        checkString = encoding(unquote(checkString))
    response = requester(url, replaceValue(params, xsschecker, checkString, copy.deepcopy), headers, GET, request_type, delay, timeout).text.lower()
    reflectedPositions = []
