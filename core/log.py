...
def log_red_line(self, amount=65, level='INFO'):
    _switch_to_no_format_loggers(self)
    _get_level_and_log(self, red + ('-' * amount) + end, level)
    _switch_to_default_loggers(self)
    
def log_green_line(self, amount=65, level='INFO'):
    _switch_to_no_format_loggers(self)
    _get_level_and_log(self, green + ('-' * amount) + end, level)
    _switch_to_default_loggers(self)
    
def log_white_line(self, amount=90, level='INFO'):
    _switch_to_no_format_loggers(self)
    _get_level_and_log(self, white + ('-' * amount) + end, level)
    _switch_to_default_loggers(self)
    
def log_blue_line(self, amount=100, level='INFO'):
    _switch_to_no_format_loggers(self)
    _get_level_and_log(self, '\n' + blue + ('-' * amount) + end + '\n', level)
    _switch_to_default_loggers(self)
...
def setup_logger(name='xsstrike')->logging.Logger:
    ...
    # Create logger method to only log a green line
    logger.green_line = MethodType(log_green_line, logger)
    # Create logger method to only log a blue line
    logger.blue_line = MethodType(log_blue_line, logger)
    # Create logger method to only log a white line
    logger.white_line = MethodType(log_white_line, logger)
